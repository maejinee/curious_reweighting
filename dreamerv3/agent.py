...
class Agent(embodied.jax.Agent):
  ...
  def train(self, carry, data):
    ...
    metrics.update(mets)
    self.slowval.update()
    model_loss = outs['model_loss']  # >>> [CURIOUS REPLAY]
    outs = {}
    if self.config.replay_context:
      ...
      outs['replay'] = updates
    # >>> [CURIOUS REPLAY] Feed the world-model prediction error back to the
    # replay buffer as the priority signal for the Prioritized selector, so
    # rare/surprising transitions (e.g. iron, skeleton) get resampled more
    # often. stepid is always available even without replay_context, since
    # the Prioritized selector only needs stepid + priority, not the full
    # replay-context entries.
    if self.config.replay.fracs.priority > 0:
      outs['replay'] = outs.get('replay', {'stepid': stepid})
      outs['replay']['priority'] = sg(model_loss)
  ...
  def loss(self, carry, obs, prevact, training):
    ...
    # >>> [CURIOUS REPLAY] Per-timestep world-model prediction error, used as
    # the replay priority signal (config.replay.priosignal == 'model'). Taken
    # before the actor-critic/imagination losses are merged in below, so it
    # only reflects dyn/rep KL and reconstruction/reward/continue error -
    # i.e. how "surprising" each transition currently is to the world model.
    model_loss = sum(self.scales[k] * v for k, v in losses.items())
    # Observation-only diagnostics: these do not feed back into optimization
    # or sampling, but make it possible to verify that Curious Replay receives
    # a non-degenerate priority signal during long runs.
    metrics.update({
        'curious_replay/model_priority_mean': model_loss.mean(),
        'curious_replay/model_priority_std': model_loss.std(),
        'curious_replay/model_priority_max': model_loss.max(),
    })
    ...
    outs = {
        'tokens': tokens, 'repfeat': repfeat, 'losses': losses,
        'model_loss': model_loss,  # >>> [CURIOUS REPLAY]
    }
