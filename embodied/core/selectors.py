...
class Recency:
  ...
  def _sample(self, tree, rng, bfactor=16):
    ...
    for level, prob in enumerate(tree):
      ...
      for segment in path:
        p = p[segment]
      # >>> [CURIOUS REPLAY] `p` is the (already-indexed) distribution to
      # sample from at this level; `segment` is just the previous level's
      # chosen index (or unset on the first iteration) and was never the
      # right thing to take len() of.
...
class Mixture:
  ...
  def __init__(self, selectors, fractions, seed=0):
    ...
    self.rng = np.random.default_rng(seed)
    # Observation-only counters. We still draw the selector with the exact
    # original rng.choice() call below, so this does not alter replay choices.
    self.sample_counts = {key: 0 for key in keys}
    self.selector_keys = {
        id(selector): key for key, selector in zip(keys, self.selectors)}
    self.stats_lock = threading.Lock()

  def __len__(self):
    # >>> [CURIOUS REPLAY] All wrapped selectors receive the same
    # inserts/deletes (see __setitem__/__delitem__ below), so they always
    # hold the same number of items.
    return len(self.selectors[0]) if self.selectors else 0

  def __call__(self):
    selector = self.rng.choice(self.selectors, p=self.fractions)
    with self.stats_lock:
      self.sample_counts[self.selector_keys[id(selector)]] += 1
    return selector()

  def stats(self):
    with self.stats_lock:
      counts = self.sample_counts.copy()
      for key in self.sample_counts:
        self.sample_counts[key] = 0
    total = sum(counts.values())
    stats = {}
    for key, count in counts.items():
      stats[f'sample_{key}_count'] = count
      stats[f'sample_{key}_fraction'] = count / total if total else np.nan
    return stats
