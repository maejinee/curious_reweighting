...
class Replay:
  ...
  def __init__(
    ...
    # >>> [CURIOUS REPLAY] `selector or ...` would fall back to Uniform for
    # any selector that is falsy while empty (e.g. Mixture/Prioritized use
    # __len__ for that), even though a selector was explicitly given.
  ...
  def stats(self):
    ...
    priority_count = m.get('priority_count', 0)
    if priority_count:
      priority_mean = m['priority_sum'] / priority_count
      priority_var = max(
          0.0, m['priority_sq_sum'] / priority_count - priority_mean ** 2)
      stats.update({
          'priority_count': priority_count,
          'priority_mean': priority_mean,
          'priority_std': np.sqrt(priority_var),
          'priority_min': m['priority_min'],
          'priority_max': m['priority_max'],
          'priority_nonfinite': m.get('priority_nonfinite', 0),
      })
    elif 'priority_count' in m:
      stats.update({
          'priority_count': 0,
          'priority_mean': np.nan,
          'priority_std': np.nan,
          'priority_min': np.nan,
          'priority_max': np.nan,
          'priority_nonfinite': m.get('priority_nonfinite', 0),
      })
    if hasattr(self.sampler, 'stats'):
      stats.update(self.sampler.stats())
    for key in self.metrics:
      if key == 'priority_min':
        self.metrics[key] = np.inf
  ...
  def update(self, data):
    ...
    if priority is not None:
      ...
      values = np.asarray(priority, np.float64).reshape(-1)
      finite = values[np.isfinite(values)]
      self.metrics.setdefault('priority_count', 0)
      self.metrics.setdefault('priority_sum', 0.0)
      self.metrics.setdefault('priority_sq_sum', 0.0)
      self.metrics.setdefault('priority_min', np.inf)
      self.metrics.setdefault('priority_max', -np.inf)
      self.metrics.setdefault('priority_nonfinite', 0)
      self.metrics['priority_count'] += len(finite)
      self.metrics['priority_sum'] += finite.sum()
      self.metrics['priority_sq_sum'] += np.square(finite).sum()
      if len(finite):
        self.metrics['priority_min'] = min(
            self.metrics['priority_min'], finite.min())
        self.metrics['priority_max'] = max(
            self.metrics['priority_max'], finite.max())
      self.metrics['priority_nonfinite'] += len(values) - len(finite)
